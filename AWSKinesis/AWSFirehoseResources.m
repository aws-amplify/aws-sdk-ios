...
- (NSString *)definitionString {
  ...
  \"operations\":{\
    ...
    \"StartDeliveryStreamEncryption\":{\
      ...
      \"documentation\":\"<p>Enables server-side encryption (SSE) for the delivery stream. This operation is asynchronous. It returns immediately. When you invoke it, Kinesis Firehose first sets the status of the stream to <code>ENABLING</code> then to <code>ENABLED</code>. You can continue to read and write data to your stream while its status is <code>ENABLING</code> but they won't get encrypted. It can take up to 5 seconds after the encryption status changes to <code>ENABLED</code> before all records written to the delivery stream are encrypted.</p> <p>To check the encryption state of a delivery stream, use <a>DescribeDeliveryStream</a>. </p> <p>You can only enable SSE for a delivery stream that uses <code>DirectPut</code> as its source. </p> <p>The <code>StartDeliveryStreamEncryption</code> and <code>StopDeliveryStreamEncryption</code> operations have a combined limit of 25 calls per delivery stream per 24 hours. For example, you reach the limit if you call <code>StartDeliveryStreamEncryption</code> thirteen times and <code>StopDeliveryStreamEncryption</code> twelve times for the same stream in a 24-hour period.</p>\"\
    },\
    \"StopDeliveryStreamEncryption\":{\
      ...
      \"documentation\":\"<p>Disables server-side encryption (SSE) for the delivery stream. This operation is asynchronous. It returns immediately. When you invoke it, Kinesis Firehose first sets the status of the stream to <code>DISABLING</code> then to <code>DISABLED</code>. You can continue to read and write data to your stream while its status is <code>DISABLING</code>. It can take up to 5 seconds after the encryption status changes to <code>DISABLED</code> before all records written to the delivery stream are no longer subject to encryption.</p> <p>To check the encryption state of a delivery stream, use <a>DescribeDeliveryStream</a>. </p> <p>The <code>StartDeliveryStreamEncryption</code> and <code>StopDeliveryStreamEncryption</code> operations have a combined limit of 25 calls per delivery stream per 24 hours. For example, you reach the limit if you call <code>StartDeliveryStreamEncryption</code> thirteen times and <code>StopDeliveryStreamEncryption</code> twelve times for the same stream in a 24-hour period.</p>\"\
    },\
    \"TagDeliveryStream\":{\
    ...
    \"KinesisStreamSourceDescription\":{\
      ...
      \"members\":{\
        ...
        \"DeliveryStartTimestamp\":{\
          ...
          \"documentation\":\"<p>Kinesis Data Firehose starts retrieving records from the Kinesis data stream starting with this time stamp.</p>\"\
        }\
      },\
