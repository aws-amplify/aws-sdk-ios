...
- (void) test_ConfigurationRestartValues
{
    ...
    id mockConfiguration = [OCMockObject niceMockForProtocol:@protocol(AWSMobileAnalyticsConfiguration)];
...
-(void)test_StartSession_SessionIsActive_DoesNotReplaceGlobalAttribute_IfWithinTimeInterval{
    ...
    AWSMobileAnalytics* insights = [AWSMobileAnalytics mobileAnalyticsWithEventRecorderService:ers appNamespace:TEST_APP_KEY completionBlock:nil];
...
-(void)test_StartSession_PreviousPersistedSession_FiresStopEvent_FiresStartEvent{
    ...
    insights = [AWSMobileAnalytics mobileAnalyticsWithEventRecorderService:ers appNamespace:TEST_APP_KEY completionBlock:^(AWSMobileAnalytics *newInsights) {
    ...
    assertThatBool(duration >= 5000, is(equalToBool(YES)));
    assertThatBool(duration <= 7000, is(equalToBool(YES)));
    assertThat([[interceptor secondToLastEvent] attributeForKey:SESSION_START_TIME_ATTRIBUTE_KEY], is(equalTo([[NSDate dateWithTimeIntervalSince1970:[persistedStartTimeMillis doubleValue]/1000] az_stringValue:AZDateISO8601DateFormat3])));
    assertThat([[interceptor secondToLastEvent] attributeForKey:SESSION_END_TIME_ATTRIBUTE_KEY], is(equalTo([[NSDate dateWithTimeIntervalSince1970:[persistedStopTimeMillis doubleValue]/1000] az_stringValue:AZDateISO8601DateFormat3])));
