...
- (void)testVerifyResourcesExistForTagris {
    ...
    XCTAssertNotNil(mockNetworking);
    [awsClient setValue:mockNetworking forKey:@"networking"];
    [[[[AWSiotevents ioteventsForKey:key] verifyResourcesExistForTagris:[AWSioteventsTagrisVerifyResourcesExistInput new]] continueWithBlock:^id(AWSTask *task) {
        XCTAssertNotNil(task.error);
        XCTAssertEqualObjects(@"OCMockExpectedNetworkingError", task.error.domain);
        XCTAssertEqual(8848, task.error.code);
        XCTAssertNil(task.result);
...
- (void)testVerifyResourcesExistForTagrisCompletionHandler {
	...
	[[AWSiotevents ioteventsForKey:key] verifyResourcesExistForTagris:[AWSioteventsTagrisVerifyResourcesExistInput new] completionHandler:^(AWSioteventsTagrisVerifyResourcesExistOutput* _Nullable response, NSError * _Nullable error) {
        XCTAssertNotNil(error);
        XCTAssertEqualObjects(@"OCMockExpectedNetworkingError", error.domain);
        XCTAssertEqual(8848, error.code);
        XCTAssertNil(response);
