...
- (NSString *)definitionString {
  ...
  \"operations\":{\
    ...
    \"AttachLoadBalancers\":{\
      ...
      \"documentation\":\"<note> <p>This API call has been replaced with a new \\\"traffic sources\\\" API call (<code>AttachTrafficSources</code>) that can attach multiple traffic sources types. While we continue to support <code>AttachLoadBalancers</code>, and you can use both the original <code>AttachLoadBalancers</code> API call and the new <code>AttachTrafficSources</code> API call on the same Auto Scaling group, we recommend using the new \\\"traffic sources\\\" API call to simplify how you manage traffic sources.</p> </note> <p>Attaches one or more Classic Load Balancers to the specified Auto Scaling group. Amazon EC2 Auto Scaling registers the running instances with these Classic Load Balancers.</p> <p>To describe the load balancers for an Auto Scaling group, call the <a>DescribeLoadBalancers</a> API. To detach a load balancer from the Auto Scaling group, call the <a>DetachLoadBalancers</a> API.</p> <p>This operation is additive and does not detach existing Classic Load Balancers or target groups from the Auto Scaling group.</p> <p>For more information, see <a href=\\\"https://docs.aws.amazon.com/autoscaling/ec2/userguide/autoscaling-load-balancer.html\\\">Use Elastic Load Balancing to distribute traffic across the instances in your Auto Scaling group</a> in the <i>Amazon EC2 Auto Scaling User Guide</i>.</p>\"\
    },\
    \"AttachTrafficSources\":{\
      ...
      \"documentation\":\"<p>Attaches one or more traffic sources to the specified Auto Scaling group.</p> <p>You can use any of the following as traffic sources for an Auto Scaling group:</p> <ul> <li> <p>Application Load Balancer</p> </li> <li> <p>Classic Load Balancer</p> </li> <li> <p>Network Load Balancer</p> </li> <li> <p>Gateway Load Balancer</p> </li> <li> <p>VPC Lattice</p> </li> </ul> <p>This operation is additive and does not detach existing traffic sources from the Auto Scaling group. </p> <p>After the operation completes, use the <a>DescribeTrafficSources</a> API to return details about the state of the attachments between traffic sources and your Auto Scaling group. To detach a traffic source from the Auto Scaling group, call the <a>DetachTrafficSources</a> API.</p>\"\
    },\
    ...
    \"DescribeLoadBalancers\":{\
      ...
      \"documentation\":\"<note> <p>This API call has been replaced with a new \\\"traffic sources\\\" API call (<code>DescribeTrafficSources</code>) that can describe multiple traffic sources types. While we continue to support <code>DescribeLoadBalancers</code>, and you can use both the original <code>DescribeLoadBalancers</code> API call and the new <code>DescribeTrafficSources</code> API call on the same Auto Scaling group, we recommend using the new \\\"traffic sources\\\" API call to simplify how you manage traffic sources.</p> </note> <p>Gets information about the load balancers for the specified Auto Scaling group.</p> <p>This operation describes only Classic Load Balancers. If you have Application Load Balancers, Network Load Balancers, or Gateway Load Balancers, use the <a>DescribeLoadBalancerTargetGroups</a> API instead.</p> <p>To determine the attachment status of the load balancer, use the <code>State</code> element in the response. When you attach a load balancer to an Auto Scaling group, the initial <code>State</code> value is <code>Adding</code>. The state transitions to <code>Added</code> after all Auto Scaling instances are registered with the load balancer. If Elastic Load Balancing health checks are enabled for the Auto Scaling group, the state transitions to <code>InService</code> after at least one Auto Scaling instance passes the health check. When the load balancer is in the <code>InService</code> state, Amazon EC2 Auto Scaling can terminate and replace any instances that are reported as unhealthy. If no registered instances pass the health checks, the load balancer doesn't enter the <code>InService</code> state. </p> <p>Load balancers also have an <code>InService</code> state if you attach them in the <a>CreateAutoScalingGroup</a> API call. If your load balancer state is <code>InService</code>, but it is not working properly, check the scaling activities by calling <a>DescribeScalingActivities</a> and take any corrective actions necessary.</p> <p>For help with failed health checks, see <a href=\\\"https://docs.aws.amazon.com/autoscaling/ec2/userguide/ts-as-healthchecks.html\\\">Troubleshooting Amazon EC2 Auto Scaling: Health checks</a> in the <i>Amazon EC2 Auto Scaling User Guide</i>. For more information, see <a href=\\\"https://docs.aws.amazon.com/autoscaling/ec2/userguide/autoscaling-load-balancer.html\\\">Use Elastic Load Balancing to distribute traffic across the instances in your Auto Scaling group</a> in the <i>Amazon EC2 Auto Scaling User Guide</i>. </p>\"\
    },\
    ...
    \"DetachLoadBalancerTargetGroups\":{\
      ...
      \"documentation\":\"<note> <p>This API call has been replaced with a new \\\"traffic sources\\\" API call (<code>DetachTrafficSources</code>) that can detach multiple traffic sources types. While we continue to support <code>DetachLoadBalancerTargetGroups</code>, and you can use both the original <code>DetachLoadBalancerTargetGroups</code> API call and the new <code>DetachTrafficSources</code> API call on the same Auto Scaling group, we recommend using the new \\\"traffic sources\\\" API call to simplify how you manage traffic sources.</p> </note> <p>Detaches one or more target groups from the specified Auto Scaling group.</p> <p>When you detach a target group, it enters the <code>Removing</code> state while deregistering the instances in the group. When all instances are deregistered, then you can no longer describe the target group using the <a>DescribeLoadBalancerTargetGroups</a> API call. The instances remain running.</p> <note> <p>You can use this operation to detach target groups that were attached by using <a>AttachLoadBalancerTargetGroups</a>, but not for target groups that were attached by using <a>AttachTrafficSources</a>.</p> </note>\"\
    },\
    \"DetachLoadBalancers\":{\
      ...
      \"documentation\":\"<note> <p>This API call has been replaced with a new \\\"traffic sources\\\" API call (<code>DetachTrafficSources</code>) that can detach multiple traffic sources types. While we continue to support <code>DetachLoadBalancers</code>, and you can use both the original <code>DetachLoadBalancers</code> API call and the new <code>DetachTrafficSources</code> API call on the same Auto Scaling group, we recommend using the new \\\"traffic sources\\\" API call to simplify how you manage traffic sources.</p> </note> <p>Detaches one or more Classic Load Balancers from the specified Auto Scaling group.</p> <p>This operation detaches only Classic Load Balancers. If you have Application Load Balancers, Network Load Balancers, or Gateway Load Balancers, use the <a>DetachLoadBalancerTargetGroups</a> API instead.</p> <p>When you detach a load balancer, it enters the <code>Removing</code> state while deregistering the instances in the group. When all instances are deregistered, then you can no longer describe the load balancer using the <a>DescribeLoadBalancers</a> API call. The instances remain running.</p>\"\
    },\
  ...
  \"shapes\":{\
    ...
    \"AutoScalingGroup\":{\
      ...
      \"members\":{\
        ...
        \"HealthCheckType\":{\
          ...
          \"documentation\":\"<p>A comma-separated list of one or more health check types.</p>\"\
        },\
    ...
    \"DetachTrafficSourcesType\":{\
      ...
      \"members\":{\
        ...
        \"TrafficSources\":{\
          \"shape\":\"TrafficSources\",\
          \"documentation\":\"<p>The unique identifiers of one or more traffic sources you are detaching. You can specify up to 10 traffic sources.</p>\"\
        }\
      }\
    },\
