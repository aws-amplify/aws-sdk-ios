...
/**
 <note><p>This API call has been replaced with a new "traffic sources" API call (<code>AttachTrafficSources</code>) that can attach multiple traffic sources types. While we continue to support <code>AttachLoadBalancers</code>, and you can use both the original <code>AttachLoadBalancers</code> API call and the new <code>AttachTrafficSources</code> API call on the same Auto Scaling group, we recommend using the new "traffic sources" API call to simplify how you manage traffic sources.</p></note><p>Attaches one or more Classic Load Balancers to the specified Auto Scaling group. Amazon EC2 Auto Scaling registers the running instances with these Classic Load Balancers.</p><p>To describe the load balancers for an Auto Scaling group, call the <a>DescribeLoadBalancers</a> API. To detach a load balancer from the Auto Scaling group, call the <a>DetachLoadBalancers</a> API.</p><p>This operation is additive and does not detach existing Classic Load Balancers or target groups from the Auto Scaling group.</p><p>For more information, see <a href="https://docs.aws.amazon.com/autoscaling/ec2/userguide/autoscaling-load-balancer.html">Use Elastic Load Balancing to distribute traffic across the instances in your Auto Scaling group</a> in the <i>Amazon EC2 Auto Scaling User Guide</i>.</p>
...
/**
 <note><p>This API call has been replaced with a new "traffic sources" API call (<code>DetachTrafficSources</code>) that can detach multiple traffic sources types. While we continue to support <code>DetachLoadBalancerTargetGroups</code>, and you can use both the original <code>DetachLoadBalancerTargetGroups</code> API call and the new <code>DetachTrafficSources</code> API call on the same Auto Scaling group, we recommend using the new "traffic sources" API call to simplify how you manage traffic sources.</p></note><p>Detaches one or more target groups from the specified Auto Scaling group.</p><p>When you detach a target group, it enters the <code>Removing</code> state while deregistering the instances in the group. When all instances are deregistered, then you can no longer describe the target group using the <a>DescribeLoadBalancerTargetGroups</a> API call. The instances remain running.</p><note><p>You can use this operation to detach target groups that were attached by using <a>AttachLoadBalancerTargetGroups</a>, but not for target groups that were attached by using <a>AttachTrafficSources</a>.</p></note>
...
/**
 <note><p>This API call has been replaced with a new "traffic sources" API call (<code>DetachTrafficSources</code>) that can detach multiple traffic sources types. While we continue to support <code>DetachLoadBalancers</code>, and you can use both the original <code>DetachLoadBalancers</code> API call and the new <code>DetachTrafficSources</code> API call on the same Auto Scaling group, we recommend using the new "traffic sources" API call to simplify how you manage traffic sources.</p></note><p>Detaches one or more Classic Load Balancers from the specified Auto Scaling group.</p><p>This operation detaches only Classic Load Balancers. If you have Application Load Balancers, Network Load Balancers, or Gateway Load Balancers, use the <a>DetachLoadBalancerTargetGroups</a> API instead.</p><p>When you detach a load balancer, it enters the <code>Removing</code> state while deregistering the instances in the group. When all instances are deregistered, then you can no longer describe the load balancer using the <a>DescribeLoadBalancers</a> API call. The instances remain running.</p>
