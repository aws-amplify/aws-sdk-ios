...
- (void)dealloc {
    ...
    // Enhanced logging for crash analysis during deallocation
...
- (instancetype)initWithSession:(nonnull AWSMQTTSession *)session
    ...
    if (self = [super init]) {
        ...
        AWSDDLogInfo(@"[CRASH-ANALYSIS] AWSIoTStreamThread created - Thread: %@, HasOutputStream: %@", 
                     self, outputStream ? @"YES" : @"NO");
        
        // Background transition tracking is handled externally
    }
    return self;
}
...
- (void)main {
    ...
    AWSDDLogInfo(@"[CRASH-ANALYSIS] Started execution of Thread: [%@], Creation-to-Start Delay: %.3fs, Background Transitions: %lu", 
                 self, creationToStartDelay, (unsigned long)self.backgroundTransitionCount);
    ...
    while ([self shouldContinueRunning]) {
        ...
        if ([now timeIntervalSinceDate:lastLogTime] >= 30.0) {
            AWSDDLogInfo(@"[CRASH-ANALYSIS] Thread health check - Thread: %@, RunLoop Cycles: %lu, Background Transitions: %lu, IsRunning: %@, IsCancelled: %@", 
                         self, (unsigned long)runLoopCycles, (unsigned long)self.backgroundTransitionCount,
...
- (void)cancel {
    AWSDDLogInfo(@"[CRASH-ANALYSIS] Issued Cancel on thread [%@], Background Transitions: %lu, HasSerialQueue: %@, IsDeallocationInProgress: %@", 
                 (NSThread *)self, (unsigned long)self.backgroundTransitionCount,
...
- (void)cancelAndDisconnect:(BOOL)shouldDisconnect {
    AWSDDLogInfo(@"[CRASH-ANALYSIS] Issued Cancel and Disconnect = [%@] on thread [%@], Background Transitions: %lu, HasSerialQueue: %@, IsDeallocationInProgress: %@", 
                 shouldDisconnect ? @"YES" : @"NO", (NSThread *)self, (unsigned long)self.backgroundTransitionCount,
                 self.serialQueue ? @"YES" : @"NO", self.isDeallocationInProgress ? @"YES" : @"NO");
...
- (void)cleanUp {
    AWSDDLogInfo(@"[CRASH-ANALYSIS] CleanUp called - Thread: %@, Background Transitions: %lu, HasSerialQueue: %@, IsDeallocationInProgress: %@, DidCleanUp: %@", 
                 self, (unsigned long)self.backgroundTransitionCount,
