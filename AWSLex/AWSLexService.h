...
//! SDK version for AWSLex
FOUNDATION_EXPORT NSString *const AWSLexSDKVersion;
...
/**
 ...
 @param request A container for the necessary parameters to execute the DeleteSession service method.

 @return An instance of `AWSTask`. On successful execution, `task.result` will contain an instance of `AWSLexDeleteSessionResponse`. On failed execution, `task.error` may contain an `NSError` with `AWSLexErrorDomain` domain and the following error code: `AWSLexErrorNotFound`, `AWSLexErrorBadRequest`, `AWSLexErrorLimitExceeded`, `AWSLexErrorInternalFailure`, `AWSLexErrorConflict`.
 
 @see AWSLexDeleteSessionRequest
 @see AWSLexDeleteSessionResponse
 */
...
/**
 ...
 @param request A container for the necessary parameters to execute the GetSession service method.

 @return An instance of `AWSTask`. On successful execution, `task.result` will contain an instance of `AWSLexGetSessionResponse`. On failed execution, `task.error` may contain an `NSError` with `AWSLexErrorDomain` domain and the following error code: `AWSLexErrorNotFound`, `AWSLexErrorBadRequest`, `AWSLexErrorLimitExceeded`, `AWSLexErrorInternalFailure`.
 
 @see AWSLexGetSessionRequest
 @see AWSLexGetSessionResponse
 */
...
/**
 ...
 @param completionHandler The completion handler to call when the load request is complete.
                          `response` - A response object, or `nil` if the request failed.
                          `error` - An error object that indicates why the request failed, or `nil` if the request was successful. On failed execution, `error` may contain an `NSError` with `AWSLexErrorDomain` domain and the following error code: `AWSLexErrorNotFound`, `AWSLexErrorBadRequest`, `AWSLexErrorLimitExceeded`, `AWSLexErrorInternalFailure`.
...
/**
 ...
 @param request A container for the necessary parameters to execute the PutSession service method.

 @return An instance of `AWSTask`. On successful execution, `task.result` will contain an instance of `AWSLexPutSessionResponse`. On failed execution, `task.error` may contain an `NSError` with `AWSLexErrorDomain` domain and the following error code: `AWSLexErrorNotFound`, `AWSLexErrorBadRequest`, `AWSLexErrorLimitExceeded`, `AWSLexErrorInternalFailure`, `AWSLexErrorConflict`, `AWSLexErrorNotAcceptable`, `AWSLexErrorDependencyFailed`, `AWSLexErrorBadGateway`.
 
 @see AWSLexPutSessionRequest
 @see AWSLexPutSessionResponse
 */
