...
@interface AWSAlexaforbusinessResources ()

@property (nonatomic, strong) NSDictionary *definitionDictionary;

@end

@implementation AWSAlexaforbusinessResources

+ (instancetype)sharedInstance {
    static AWSAlexaforbusinessResources *_sharedResources = nil;
    static dispatch_once_t once_token;

    dispatch_once(&once_token, ^{
        _sharedResources = [AWSAlexaforbusinessResources new];
    });
...
- (NSString *)definitionString {
  ...
  \"shapes\":{\
    ...
    \"CreateMeetingRoomConfiguration\":{\
      ...
      \"members\":{\
        ...
        \"RequireCheckIn\":{\
          ...
          \"documentation\":\"<p>Settings for requiring a check in when a room is reserved. Alexa can cancel a room reservation if it's not checked into to make the room available for others. Users can check in by joining the meeting with Alexa or an AVS device, or by saying âAlexa, check in.â</p>\"\
        }\
      },\
    ...
    \"CreateRequireCheckIn\":{\
      ...
      \"documentation\":\"<p>Creates settings for the require check in feature that are applied to a room profile. Require check in allows a meeting roomâs Alexa or AVS device to prompt the user to check in; otherwise, the room will be released.</p>\"\
    },\
    \"CreateRoomRequest\":{\
      \"type\":\"structure\",\
      \"required\":[\"RoomName\"],\
    ...
    \"DeviceStatusDetail\":{\
      ...
      \"documentation\":\"<p>Details of a deviceâs status.</p>\"\
    },\
    ...
    \"MeetingRoomConfiguration\":{\
      ...
      \"members\":{\
        ...
        \"RequireCheckIn\":{\
          ...
          \"documentation\":\"<p>Settings for requiring a check in when a room is reserved. Alexa can cancel a room reservation if it's not checked into. This makes the room available for others. Users can check in by joining the meeting with Alexa or an AVS device, or by saying âAlexa, check in.â </p>\"\
        }\
      },\
    ...
    \"PutSkillAuthorizationRequest\":{\
      ...
      \"members\":{\
        \"AuthorizationResult\":{\
          ...
          \"documentation\":\"<p>The authorization result specific to OAUTH code grant output. \\\"Codeâ must be populated in the AuthorizationResult map to establish the authorization.</p>\"\
        },\
        \"SkillId\":{\
          \"shape\":\"SkillId\",\
          \"documentation\":\"<p>The unique identifier of a skill.</p>\"\
    ...
    \"RequireCheckIn\":{\
      ...
      \"documentation\":\"<p>Settings for the require check in feature that are applied to a room profile. Require check in allows a meeting roomâs Alexa or AVS device to prompt the user to check in; otherwise, the room will be released. </p>\"\
    },\
    \"RequirePin\":{\
      \"type\":\"string\",\
      \"enum\":[\
    ...
    \"RoomSkillParameter\":{\
      ...
      \"members\":{\
        \"ParameterKey\":{\
          \"shape\":\"RoomSkillParameterKey\",\
          \"documentation\":\"<p>The parameter key of a room skill parameter. ParameterKey is an enumerated type that only takes âDEFAULTâ or âSCOPEâ as valid values.</p>\"\
        },\
        \"ParameterValue\":{\
    ...
    \"UpdateRequireCheckIn\":{\
      ...
      \"documentation\":\"<p>Updates settings for the require check in feature that are applied to a room profile. Require check in allows a meeting roomâs Alexa or AVS device to prompt the user to check in; otherwise, the room will be released. </p>\"\
    },\
    \"UpdateRoomRequest\":{\
      \"type\":\"structure\",\
      \"members\":{\
