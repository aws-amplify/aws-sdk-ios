 ...
 *
 * @return An instance of BFTask. On successful execution, task.result will contain an instance of AWSSQSSendMessageResult. On failed execution, task.error may contain an NSError with AWSSQSErrorDomain domian and the following error code: AWSSQSErrorInvalidMessageContents.
 *
...
/**
 * <p>Sets the value of one or more queue attributes.</p><note>Going forward, new attributes might be added.If you are writing code that calls this action, we recommend that you structure your code so that it can handle new attributes gracefully.</note>
 *
