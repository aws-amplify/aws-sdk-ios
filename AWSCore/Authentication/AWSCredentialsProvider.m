...
- (AWSTask<NSNumber *> *)updateIdentityIdWithAuthLogins:(NSDictionary<NSString *,NSString *> *)logins {
    ...
    if (self.useEnhancedFlow) {
        ...
        if (self.identityId == nil) {
            ...
            // getIdentityFromCredentials api.
            return [[self getIdentityFromCredentialsForIdentityAPI:logins] continueWithBlock:^id (AWSTask *task) {
                if (task.error) {
                    
                    // If the api returned an error, we first check if the error is related to some case where
                    // the migration of identity id is not possible, we should try to get the identity id using
                    // the getIdentityId api and discard the existing identity id.
                    if (![AWSCognitoCredentialsProvider shouldResetIdentityId:task.error authenticated:true]) {
                        return task;
                    }
                    
                    AWSDDLogDebug(@"updateIdentityIdWithAuthLogins: Resetting identity Id and calling getIdentityId");
                    self.identityId = nil;
                    providerRef.identityId = nil;
                    return [[self getIdentityIdAPI:logins] continueWithSuccessBlock:^id (AWSTask *task) {
                        NSString *identityIdFromResponse = task.result;
...
- (AWSTask<NSString *> *)getIdentityFromCredentialsForIdentityAPI:(NSDictionary<NSString *,NSString *> *)logins {
    ...
    getCredentialsInput.logins = logins;
    getCredentialsInput.customRoleArn = customRoleArn;
    return [[self.cognitoIdentity getCredentialsForIdentity:getCredentialsInput] continueWithSuccessBlock:^id (AWSTask *task) {
        return [AWSTask taskWithResult:[task.result identityId]];
    }];
}

- (AWSTask<NSString *> *) getIdentityIdAPI:(NSDictionary<NSString *,NSString *> *)logins {
    ...
    return [[self.cognitoIdentity getId:getIdInput] continueWithSuccessBlock:^id (AWSTask *task) {
        return [AWSTask taskWithResult:[task.result identityId]];
    }];
}
